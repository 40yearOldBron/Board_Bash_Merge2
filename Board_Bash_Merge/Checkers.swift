...
struct CheckersView: View {
    ...
    @State private var isRedTurn = true // true = BLUE (player), false = RED (CPU)
    @State var moveTimer: Timer? = nil
    @State var timeRemaining = 30.0
    @State var timerFinished = false
    @State private var isMoving = false   // 🔒 LOCK
    ...
    func loadGame() {
        if let data = UserDefaults.standard.data(forKey: "savedPieces"),
    ...
    // MARK: - 🔥 FIXED MOVE ENGINE (THIS WAS YOUR BUG)
    func movePiece(by id: UUID, toRow: Int, toCol: Int) {
        ...
        if abs(toRow - piece.row) == 2 {
            ...
            if let capturedIndex = board.firstIndex(where: {
        ...
        board[index].row = toRow
        board[index].col = toCol
        
        if (board[index].isRed && toRow == 0) ||
           (!board[index].isRed && toRow == 7) {
    ...
    func makeAIMove() {
        ...
        for piece in cpuPieces {
            for row in 0..<8 {
                for col in 0..<8 {
                    ...
                    if isValidMove(from: piece, toRow: row, toCol: col) {
                        if abs(row - piece.row) == 2 {
                            captureMoves.append((piece.id, row, col))
                        } else {
                            normalMoves.append((piece.id, row, col))
                        }
                    }
                }
            }
        ...
        guard let move = (captureMoves.isEmpty ? normalMoves : captureMoves).randomElement() else {
            isRedTurn = true
            isMoving = false
            return
        }
